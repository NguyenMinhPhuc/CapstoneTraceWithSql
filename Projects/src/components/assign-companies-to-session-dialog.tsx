...
export function AssignCompaniesToSessionDialog({
  ...
  const { data: sessions, isLoading: isLoadingSessions } =
    useCollection<GraduationDefenseSession>(sessionsCollectionRef);
    ...
    (async () => {
      ...
      try {
        ...
        const mapped = (list || []).map(
          (c: any) => ({ id: String(c.id), name: c.name } as InternshipCompany)
        );
        if (!mounted) return;
  ...
  return (
    <DialogContent className="sm:max-w-md">
      <DialogHeader>
        ...
        <DialogDescription>
          Chọn một đợt báo cáo để gán {companiesToAssign.length} doanh nghiệp đã
          chọn.
        </DialogDescription>
      </DialogHeader>
      ...
      <div className="space-y-4 py-4">
        ...
        >
          <SelectTrigger>
            <SelectValue
              placeholder={
                isLoadingSessions ? "Đang tải..." : "Chọn đợt báo cáo"
              }
            />
          </SelectTrigger>
          <SelectContent>
      ...
      <DialogFooter>
        ...
        <Button
          ...
          disabled={
            isSubmitting || companiesToAssign.length === 0 || !selectedSessionId
          }
        >
          {isSubmitting
            ? "Đang gán..."
            : `Gán ${companiesToAssign.length} doanh nghiệp`}
        </Button>
      </DialogFooter>
    </DialogContent>
  );
