...
export function EarlyInternshipForm({
  ...
  const [lhuDepartments, setLhuDepartments] = useState<
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const list = await companiesService.getAll({ company_type: "lhu" });
        ...
        if (!mounted) return;
        setLhuDepartments(mapped);
      } catch (err) {
        console.error("Failed to load LHU departments from backend:", err);
        if (mounted) setLhuDepartments([]);
      } finally {
