...
export function EditMajorForm({ major, onFinished }: EditMajorFormProps) {
  ...
  const onSubmit = async (data: MajorFormData) => {
    try {
      await majorsService.update(major.id, data);
      toast({
        title: "Thành công",
        description: "Đã cập nhật thông tin ngành",
      });
      onFinished();
    } catch (error: any) {
      toast({
        variant: "destructive",
        title: "Lỗi",
        description: error?.message || "Không thể cập nhật ngành",
      });
    }
  };
