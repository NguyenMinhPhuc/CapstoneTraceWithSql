...
const criterionSchema = z.object({
  id: z.string(), // UI-only
  criterionId: z.number().optional(), // DB id if exists
...
export function EditRubricForm({ rubricId, onFinished }: EditRubricFormProps) {
  const { toast } = useToast();
  const [loading, setLoading] = useState(true);
  const [initialCriteriaIds, setInitialCriteriaIds] = useState<number[]>([]);
  ...
  useEffect(() => {
    async function load() {
      setLoading(true);
      try {
        const rubric = await rubricsService.getRubric(rubricId);
        form.reset({
          name: rubric.name,
        ...
        setInitialCriteriaIds(rubric.criteria.map((c) => c.id));
      } catch (error: any) {
        toast({
          variant: "destructive",
          title: "Lỗi",
          description: error?.message || "Không tải được rubric",
        });
      } finally {
        setLoading(false);
      }
  ...
  async function onSubmit(values: RubricFormData) {
    try {
      ...
      await rubricsService.updateRubric(rubricId, {
        name: values.name,
      ...
      for (let i = 0; i < values.criteria.length; i++) {
        ...
        if ((c as any).criterionId) {
          presentIds.push((c as any).criterionId);
          await rubricsService.updateCriterion(
            rubricId,
            (c as any).criterionId,
            {
              name: c.name,
          ...
          const created = await rubricsService.addCriterion(rubricId, {
            name: c.name,
            description: c.description || null,
            PLO: (c as any).PLO ?? null,
            PI: (c as any).PI ?? null,
          ...
          presentIds.push(created.id);
        }
      }

      for (const initId of initialCriteriaIds) {
        if (!presentIds.includes(initId)) {
          await rubricsService.deleteCriterion(rubricId, initId);
        }
      }
      ...
      toast({
        variant: "destructive",
        title: "Lỗi",
        description: `Không thể cập nhật rubric: ${
