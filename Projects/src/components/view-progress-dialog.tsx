...
export function ViewProgressDialog({
  ...
  const [reports, setReports] = useState<WeeklyProgressReport[] | null>(null);
  ...
  useEffect(() => {
    ...
    async function load() {
      setIsLoading(true);
      try {
        const data = await progressService.getWeeklyReportsByRegistration(
          registration.id as string
        );
        if (mounted) setReports(data || []);
      } catch (e) {
        console.error("Failed to load progress reports via API", e);
        setReports([]);
      } finally {
  ...
  const handleAction = async (
    ...
    try {
      await progressService.updateWeeklyReport(reportId, {
        status,
      ...
      console.error("Error updating progress report via API:", error);
      toast({
        variant: "destructive",
        title: "Lỗi",
  ...
  return (
    <>
      <DialogHeader>
        ...
        <DialogDescription>
          Xem và duyệt báo cáo của sinh viên: {registration.studentName} (
        ...
        <div className="py-4 space-y-4">
          {isLoading ? (
                      ...
                      {report.status === "pending_review" ? (
                        ...
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground italic">
                            "{report.supervisorComments || "Không có nhận xét."}
                            "
                          </p>
                        </div>
                      )}
            ...
            <Alert>
              ...
              <AlertDescription>
                Sinh viên này chưa nộp báo cáo tiến độ nào.
              </AlertDescription>
            </Alert>
          )}
        </div>
