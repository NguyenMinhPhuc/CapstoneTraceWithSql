...
export function ViewStudentEarlyInternshipProgressDialog({
  internship,
  reports,
  onFinished,
}: ViewStudentEarlyInternshipProgressDialogProps) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const setting = await settingsService.get("features");
        if (!mounted) return;
        const parsed = setting?.value ? JSON.parse(setting.value) : null;
        setGoalHours(parsed?.earlyInternshipGoalHours ?? 700);
      } catch (e) {
        console.error("Failed to load settings.features via API", e);
      }
    })();
    return () => {
      mounted = false;
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            ...
            <p className="text-sm text-muted-foreground">
              {progress.totalHours.toFixed(0)}/{progress.goalHours} giờ
            </p>
          </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <AccordionContent className="space-y-2">
                        <p className="text-sm">
                          <b>Số giờ đã báo cáo:</b> {report.hours}
                        </p>
                        <p className="text-sm text-muted-foreground italic">
                          "{report.supervisorComments || "Chưa có nhận xét."}"
                        </p>
                      </AccordionContent>
                    </AccordionItem>
                  );
